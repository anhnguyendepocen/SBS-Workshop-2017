...
\item Read the data into \texttt{R}, saving it in an object named
  \texttt{sports.df}.
<<eval = F>>=
sports.df <- read.csv("location of your folder/sports.csv", 
...
<<>>=
mean(sports.df$age, na.rm = TRUE)
sd(sports.df$age, na.rm = TRUE)
@
...
<<>>=
gen.eth.tab <- with(sports.df, table(ethnicity, gender))
round(prop.table(gen.eth.tab, 2)*100, 1)
@

\item Now turn the frequency table in \ref{sec:read}(\ref{itm:2freq})
  into row proportions, keep only 1 decimal place.
<<>>=
round(prop.table(gen.eth.tab, 1)*100, 1)
@
